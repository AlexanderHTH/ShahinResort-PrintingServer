...
public class PrintedAuthorizationService(ITQLogger<PrintedAuthorizationService> logger,
                                          IUserContext userContext) : IEntityAuthorizationService<Printed>
{
    public bool Authorize(ResourceOperation operation, Printed? entity = null)
    {
        var user = userContext.GetCurrentUser();
        if (user != null)
        {
            logger.LogInformation("Authorizing user {UserName}, to {Operation} for printed {Name}", user.Email, operation, entity == null ? "" : entity.Id.ToString());
