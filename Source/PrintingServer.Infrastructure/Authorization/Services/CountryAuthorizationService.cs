...
public class CountryAuthorizationService(ITQLogger<CountryAuthorizationService> logger,
                                         IUserContext userContext) : IEntityAuthorizationService<Country>
{
    public bool Authorize(ResourceOperation operation, Country? entity = null)
    {
        var user = userContext.GetCurrentUser();
        if (user != null)
        {
            logger.LogInformation("Authorizing user {UserName}, to {Operation} for country {Name}", user.Email, operation, entity == null ? "" : entity.Id.ToString());
